...
const StatCounter = ({ value, suffix, statsVisible }: { value: number; suffix: string; statsVisible: boolean }) => {
  const count = useCountUp(value, 2000, statsVisible);
  return <>{count}{suffix}</>;
};
            ...
            {subHeroConfig.stats.map((stat, index) => (
              <div key={index} className={`p-6 ${index < subHeroConfig.stats.length - 1 ? 'border-r border-gray-200' : ''}`}>
                <span className="block font-serif text-4xl md:text-5xl text-[#8b6d4b] mb-2">
                  <StatCounter value={stat.value} suffix={stat.suffix} statsVisible={statsVisible} />
                </span>
